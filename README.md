# Claude Model+Harness Cog

A **Model+Harness Cog** using Claude Code with your Claude subscription login. It does not expose subscription tokens or pretend to provide bare model inference.
...
Run `claude auth login` and choose your Claude subscription. `claude auth status` confirms the login.
...
```sh
pixi run suite -- bind --provider cog-claude --request ../cog-claude/examples/bind-request.json
pixi run suite -- compose --context cog-op-designer --binding-id binding-cog-claude --revision 1
```
...
pixi run serve -- --binding ../cog-workbench/var/suite/<hash>-<revision>.json --port 8121
```
...
binding references a separately admitted model. The default port is 8121; the
sibling providers default to 8122 (cog-chatgpt) and 8123 (cog-turn-harness).

The surface is `POST /v1/chat/completions`, `GET /v1/models` and `GET /health`.
The one model id served is `claude/<binding_id>@<revision>`; a request naming
